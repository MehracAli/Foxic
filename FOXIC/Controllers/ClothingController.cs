...
namespace FOXIC.Controllers
{
	public class ClothingController : Controller
	{
		...
		public IActionResult DetailAddToBasket(int Id, ClothingVM clothingVM)
		{
			ViewBag.Sizes = _context.Sizes.ToList();
			ViewBag.Colors = _context.Colors.ToList();
		...
		public IActionResult AddWish(int Id)
		{
			...
			var Cookies = HttpContext.Request.Cookies["Wishlist"];

			CookiesWishVM wish = new();
			
			if (Cookies is null)
			{
				CookiesWihItemVM item = new()
				{
					Id = clothing.Id,
					Name = clothing.Name,
				...
				wish.cookiesWishItemVM.Add(item);
			}
			else
			{
				wish = JsonConvert.DeserializeObject<CookiesWishVM>(Cookies);
				CookiesWihItemVM exictedItem = wish.cookiesWishItemVM.Find(i => i.Id == Id);
				if (exictedItem is null)
				{
					CookiesWihItemVM item = new()
					{
			...
			var wishStr = JsonConvert.SerializeObject(wish);
			HttpContext.Response.Cookies.Append("Wishlist", wishStr);
			return RedirectToAction(nameof(Index));
		}
		...
		public IActionResult GetWish()
		{
			...
			{
				wish = JsonConvert.DeserializeObject<CookiesWishVM>(cookies);
				foreach (CookiesWihItemVM item in wish.cookiesWishItemVM)
				{
					Clothing clothing = _context.Clothes.FirstOrDefault(p => p.Id == item.Id);
					if (clothing is null)
					{
