...
namespace FOXIC.Areas.AdminFoxic.Controllers
{
	...
	public class ClothesController : Controller
	{
		...
		public IActionResult Index(string SearchString)
		{
			...
			IQueryable<Clothing> findedClothes = _context.Clothes;
			if (!String.IsNullOrEmpty(SearchString))
			{
				findedClothes = findedClothes.Where(c => c.Name.Contains(SearchString));
				return View(findedClothes);
			}
		...
		public async Task<IActionResult> Editing(int Id, Clothing editedClothing)
		{
			...
			List<string> removables = clothing.Images
				.Where(c => !editedClothing.ImagesIds
					.Contains(c.Id))
            ...
            clothing.Images.RemoveAll(p => !editedClothing.ImagesIds.Contains(p.Id));

            if (editedClothing.iff_IsMainImage != null)
			{
				...
				await CahngeIsMain(true, editedClothing.iff_IsMainImage, clothing);
			}
		...
		[HttpPost]
		[ActionName("Delete")]
		public IActionResult Deleting(int Id, Clothing deletedClothing)
		{
			if (Id == 0) return BadRequest();
			Clothing clothing = _context.Clothes.FirstOrDefault(c=>c.Id == Id);
			if (clothing == null) return NotFound();
		...
		private async Task CahngeIsMain(bool? isMain, IFormFile image, Clothing clothing)
		{
			var imagefolderPath = Path.Combine(_env.WebRootPath, "images");
